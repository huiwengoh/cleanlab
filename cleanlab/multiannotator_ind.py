...
def _get_post_pred_probs_and_weights(
    ...
    if quality_method == "crowdlab":
        if prior_pred_probs.ndim == 3:
            ...
            # print(f"anno_agreement {adjusted_annotator_agreement}")

            model_weight_arr = np.full(prior_pred_probs.shape[0], np.nan)

            for ind in range(prior_pred_probs.shape[0]):
                prior_pred_probs_subset = prior_pred_probs[ind][mask]
                ...
                model_weight_arr[ind] = model_weight

            # model_weight_arr = model_weight_arr / len(model_weight_arr)
            # print(f"model weight {model_weight_arr}")
            ...
            for i in range(len(labels_multiannotator)):
                ...
                post_pred_probs[i] = [
                    np.average(
                        [
                            prior_pred_probs[ind][i, true_label]
                            for ind in range(prior_pred_probs.shape[0])
                        ]
                        + [
                            likelihood
                        ...
                        weights=np.concatenate(
                            (model_weight_arr, adjusted_annotator_agreement[example_mask])
                        ),
                    )
                    for true_label in range(num_classes)
                ]

            return_model_weight = model_weight_arr
